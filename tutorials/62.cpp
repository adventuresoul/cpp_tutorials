...
class B : public A{     // default access specifier is private   class B : A    => private
    public:
        void f1(){
...
int main()
{
    A *aptr;
    B b1;
    aptr = &b1;     // a base class pointer can point at object of publically derived class of base class 
    
    return 0;
}

// in deriving class by public access specifier

// private members -> no class can access them except, the public member functions of the base class
// public members -> member functions of the derived class can access them. and further more the member functions of the derived class can also access them. The non member functions can also access them
